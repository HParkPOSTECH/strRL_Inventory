...
class Agent:
    """
    Structured RL for Three-item inventory system
    ...
    def __init__(self, N, s, c, S,  alpha, beta, alg_type = "SRL-FSA", stationary = "stationary", demand_dist = "gamma"):
        ...
        self.w = np.zeros((self.max_degree, self.max_degree, self.max_degree))     # initialize parameter for relative value function
        self.w[-1, 0, 0] = low_pos   # initialize the parameter for highest order of each input with small positive value
        self.w[0, -1, 0] = low_pos
        self.w[0, 0, -1] = low_pos
    ...
    def poly_basis(self, x, batch = False):
        ...
        if batch: # for batch evaluation
            basis = np.zeros((x.shape[1], self.max_degree, self.max_degree, self.max_degree), dtype=np.float32)

            for perm in product(np.arange(self.max_degree), repeat=self.N):
                if np.sum(perm) > value_degree: # over degree ==> pass
                    continue

                basis[:, perm[0], perm[1], perm[2]] = np.array([temp_xs[n, :] ** perm[n] for n in range(self.N)]) .prod(axis = 0)
                # compute term (x_1 ^i * x_2 ^ j * x_3 ^k)

        else:
            basis = np.zeros((self.max_degree, self.max_degree, self.max_degree), dtype=np.float32)
    ...
    def V(self, x, batch = False):
        ...
        if batch:   # for batch mode
            valuef = np.sum(self.w[np.newaxis] * pi, axis = (1, 2, 3))
        else:
    ...
    def update_V(self, x, r, _x):
        ...
        self.w[-1, 0, 0] = np.maximum(0., self.w[-1, 0, 0])
        self.w[0, -1, 0] = np.maximum(0., self.w[0, -1, 0])
        self.w[0, 0, -1] = np.maximum(0., self.w[0, 0, -1])    # restrict to highest order term as nonnegative value
    ...
    def update_policy(self,  x, _x, eps):
        ...
        if self.alg_type == "SRL-PSA":     # if partial stochastic approximation (PSA) based algorithm
            ...
            for perm in product(np.arange(self.max_degree), repeat = self.N):  # compute for all permutation for (# of item X degree)
                if np.sum(perm) > value_degree or np.sum(perm) == 0: # exclude for over degree or constant term
                    continue
                for n in range(self.N):
            ...
            theta1 = [self.f(x[1], self.s[1]) * self.f(x[2], self.s[2]),
                      self.f(x[2], self.s[2]) * (self.f(x[1], self.s[1]) - self.f(x[1], self.c[1])),
                      self.f(x[1], self.s[1]) * (self.f(x[2], self.s[2]) - self.f(x[2], self.c[2]))] # parameter for Multioulli for item1
            ...
            exp1 = np.where(e1 == 0, cls_c[0], np.where(e1 == 1, cls_c[1], cls_c[2]))
            self.gs[0] = self.mt * self.gs[0] + (1 - self.mt) * sbase[0] * np.mean((-1) ** exp1 * self.V(ys[0], batch = True))

            theta2 = [self.f(x[2], self.s[2]) * (self.f(x[0], self.s[0]) - self.f(x[0], self.c[0])),
                      self.f(x[0], self.s[0]) * self.f(x[2], self.s[2]),
                      self.f(x[0], self.s[0]) * (self.f(x[2], self.s[2]) - self.f(x[2], self.c[2]))]  # parameter for Multioulli for item2
            ...
            exp2 = np.where(e2 == 0, cls_c[0], np.where(e2 == 1, cls_c[1], cls_c[2]))
            self.gs[1] = self.mt * self.gs[1] + (1 - self.mt) * sbase[1] * np.mean((-1) ** exp2 * self.V(ys[1], batch = True))

            theta3 = [self.f(x[1], self.s[1]) * (self.f(x[0], self.s[0]) - self.f(x[0], self.c[0])),
                      self.f(x[0], self.s[0]) * (self.f(x[1], self.s[1]) - self.f(x[1], self.c[1])),
                      self.f(x[0], self.s[0]) * self.f(x[1], self.s[1])]  # parameter for Multioulli for item3
            ...
            exp3 = np.where(e3 == 0, cls_c[0], np.where(e3 == 1, cls_c[1], cls_c[2]))
            self.gs[2] = self.mt * self.gs[2] + (1 - self.mt) * sbase[2] * np.mean((-1) ** exp3 * self.V(ys[2], batch = True))  # momentum for updating s
